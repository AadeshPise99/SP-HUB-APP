...
import { Search, Eye, CheckCircle, XCircle, X, FileText, RotateCcw } from 'lucide-react';
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    checker_review: ['badge-checker', 'Checker Review'],
  };
...
export default function MakerInvoices() {
  ...
  const counts = {};
  TABS.forEach(t => {
    counts[t.key] = t.key === 'all' ? invoices.length : invoices.filter(i => i.status === t.key).length;
  });
  ...
  const handleAction = async (status) => {
    ...
    try {
      ...
      toast.success(status === 'approved' ? 'Invoice approved successfully' : 'Invoice rejected');
      setSelected(null);
      setRemarks('');
      fetchInvoices();
    } catch (e) {
      ...
      <div data-testid="maker-invoices">
          ...
          <div style={{ padding: '0 20px' }}>
            <div className="scf-tabs">
              {TABS.map(t => (
                <div
                  key={t.key}
                  className={`scf-tab${activeTab === t.key ? ' active' : ''}`}
                  onClick={() => setActiveTab(t.key)}
                  data-testid={`tab-${t.key}`}
                >
                  {t.label}
                  {counts[t.key] > 0 && <span className="scf-tab-count">{counts[t.key]}</span>}
                </div>
              ))}
            </div>
          </div>
            ...
            <table className="scf-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={10}>
                    <div className="scf-empty">
                      <FileText /><h3>No invoices found</h3>
                    ...
                    <tr key={inv.id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button
                            className="scf-btn scf-btn-secondary scf-btn-sm"
                            onClick={() => { setSelected(inv); setRemarks(''); }}
                          ...
                          {inv.status === 'pending_approval' && (
                            <>
                              <button
                                className="scf-btn scf-btn-success scf-btn-sm"
                                onClick={() => { setSelected(inv); setRemarks(''); }}
                                data-testid={`approve-invoice-${inv.id}`}
                              >
                                <CheckCircle size={12} />
                              </button>
                              <button
                                className="scf-btn scf-btn-danger scf-btn-sm"
                                onClick={() => { setSelected(inv); setRemarks(''); }}
                                data-testid={`reject-invoice-${inv.id}`}
                              >
                                <XCircle size={12} />
                              </button>
                            </>
        ...
        {selected && (
          <div className="scf-modal-overlay" onClick={() => setSelected(null)}>
            <div className="scf-modal-panel" onClick={e => e.stopPropagation()} data-testid="invoice-detail-panel">
              <div className="scf-modal-header">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button className="scf-modal-close" onClick={() => setSelected(null)} data-testid="close-detail-panel">
                    <X size={14} />
                  </button>
                </div>
              </div>
                      ...
                      <tbody>
                        {selected.line_items.map((li, i) => (
                          <tr key={i}>
                            ...
                            <td className="scf-amount">{fmt(li.total)}</td>
                          </tr>
                        ))}
                      </tbody>
              ...
              {selected.status === 'pending_approval' && (
                <>
                  <div className="scf-remarks-section">
                    <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#6b7280', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.3px' }}>Remarks (optional)</label>
                    <textarea
                      rows={3}
                  ...
                  <div className="scf-modal-footer">
                    <button className="scf-btn scf-btn-secondary" onClick={() => setSelected(null)} data-testid="cancel-action-btn">Cancel</button>
                    <button
                      className="scf-btn scf-btn-danger"
                    ...
                    <button
                      className="scf-btn scf-btn-success"

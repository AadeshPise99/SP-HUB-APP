        ...
        <div className="scf-table-card">
          <div style={{ padding: '0 20px' }}>
            <div className="scf-tabs">
              {TABS.map(t => (
                <div key={t.key} className={`scf-tab${activeTab === t.key ? ' active' : ''}`} onClick={() => setActiveTab(t.key)} data-testid={`cp-tab-${t.key}`}>
          ...
          <div className="scf-filter-bar">
            <div className="scf-search-wrap">
              <Search />
              <input className="scf-search-input" placeholder="Search invoices..." value={search} onChange={e => setSearch(e.target.value)} data-testid="cp-invoice-search" />
            </div>
            ...
            <div className="scf-modal-panel" onClick={e => e.stopPropagation()} data-testid="cp-invoice-detail">
              ...
              <div className="scf-modal-body">
                ...
                <div className="scf-detail-section">
                  ...
                  <div className="scf-detail-grid">
                    {[['Invoice Amount', fmt(selected.amount)], ['Discount Rate', `${selected.discount_rate}% p.a.`], ['Discount Amount', fmt(selected.discount_amount)], ['Net Disbursement', fmt(selected.net_amount)]].map(([l, v]) => (
                      <div key={l} className="scf-detail-item"><label>{l}</label><span style={{ fontWeight: l === 'Net Disbursement' ? 700 : 500 }}>{v}</span></div>
                    ))}
                  </div>
                </div>
